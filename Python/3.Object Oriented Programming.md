...
- The newly formed class is known as a `derived` class.
- The Pre Existing Class is Known as a `base` class.
- To inherit `__init__()` method of the `parent class` inside derived class, use `super().__init__()`
- `Inheritance` enables us to `define` a new `class` from existing class by adding new functionality to it or by simply modifying it.
